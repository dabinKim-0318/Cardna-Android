...
- 안드로이드 개발자가 되겠다고 결심한 이후, 패스트캠퍼스 강의를 들으면 안드로이드 개발을 독학하고 있었습니다. 혼자 공부하는 것에 한계를 느껴 여러 개발 블로그들을 찾아보았고, 많은 개발자 분들이 IT연합동아리 SOPT를 수료하셨던 걸 보았습니다. 매주 세미나와 과제를 통해 성장할 수 있는 기회가 있고 디자인, 다른 분야 개발자, 기획자 분들과 협업해 실질적인 서비스를 만들고 싶어 안드로이드 파트원으로 참여했습니다
- 남들보다 늦게 개발을 시작했다고 생각했기 때문에 더 열심히 하고 싶었습니다. 매주 세미나를 듣고 과제를 제출해야하는데 매번 배우는 새로운 기술을 직접 구현하고 바로바로 결과물을 볼 수 있는게 재미있어서 LEVEL3 과제까지 매번 하곤 했습니다. 코드리뷰도 받아보고 리뷰 반영도 하면서 점점 성장하는 제 자신을 보는게 뿌듯했고, 안드로이드 개발도 즐거워서 열심히 하다보니 과제 제출 2등이라는 결실도 얻었습니다. 
...
| Network | Retrofit2 | 안드로이드 통신 라이브러리의 역사를 공부하면서 Deprecated된 라이브러리들과(HttpClient 등) Volley, OkHttp, Retrofit의 장단점을 비교하며 공부했습니다. 그 결과 개인적으로는 Annotation으로 HTTP 메소드를 정의해서 사용하는 Retrofit이 전체 구조를 파악하기 더 좋은 것 같아 Retrofit을 선택했습니다. JSON을 파싱해주는 Converter 연동을 지원해주기 때문에 Gson 라이브러리와 함께 사용하기 위해 선택했습니다.    |
...
| Asynchronous Processing | Coroutine | api요청 시 callback을 사용하지 않고 비동기 처리를 하기 위해 Coroutine을 사용했습니다.  직관적인 함수 사용을 통해 코드의 가독성도 높일 수 있었습니다 |
...
### ◼ 첫 개발 프로젝트에서 리더 역할을 하면서..
개발을 접하기 전까지 다수의 팀 프로젝트를 해본 경험이 있습니다. 학교 팀플, 공모전, 광고 동아리 임원 등 20번이 넘는 팀 프로젝트를 경험했고 그 안에서 팀을 이끄는 역할도 종종 해왔습니다. 그런데 SOPT29기 APPJAM은 제 첫 개발 프로젝트였고, 안드로이드 파트 리더까지 맡게되어 개발을 잘 하면서 팀원들도 잘 이끌 수 있을지 걱정이 많았습니다. 지금까지 해왔던 콘텐츠 마케팅 프로젝트와는 진행방법, 세부적인 소통 방법, 데드라인 맞추기 등.. 많은 부분에서 차이가 있을 거라 생각했습니다. 안드로이드 파트가 작업을 다 끝내지 못하면 iOS, 서버팀 등 다른 파트 팀원들의 작업 일정까지 영향이 가기 때문에 안드로이드 파트 리더로서 정말 책임감을 가지고 팀을 잘 이끌기 위해 노력했습니다.

3가지 부분을 가장 노력했습니다.

1. 소통할 수 있는 분위기와 팀원간 시너지  
카드나 프로젝트의 안드로이드 파트 팀원이 정해지고 모든 팀원이 OT이후 처음 만났기 때문에 서로 친분이 없었습니다. 지금까지 프로젝트를 하면서 '서슴없이 소통하고 이야기 하는 분위기'가 프로젝트 성과에 큰 영향을 줬었기 때문에 공식 프로젝트가 시작하기 전, 팀원들끼리 친해질 수 있는 기회를 자주 만들었습니다. 다행히도 저희 팀원들의 집이 모두 가까웠기 때문에 오프라인으로 회의진행을 자주 할 수 있었던 게 서로 친해지고 편해지는데 큰 도움이 되었던 것 같습니다!!👍 (이후에는 서로 성격이 너무 잘 맞아서 회의가 아닌데도 만나서 놀기도 하고 갤럭시 팝업 스토어도 함께 가기도 했답니다ㅎㅎ) 공식 프로젝트가 끝난 후, 릴리즈를 위한 프로젝트를 진행했을 때도 자발적으로 할일을 나누고 활발하게 소통하는 팀 분위기 덕분에 대상이라는 소중한 결실도 맺고🎁 무사히 릴리즈까지 마무리 할 수 있었습니다. 이를 통해 개발 프로젝트는 혼자하는 게 아니라는 점을 다시 한 번 배웠습니다.

2. 개발 진행 과정 어깨너머로 배우기  
안드로이드 파트에는 SOPT의 OB가 없었습니다. 즉 전부 개발 프로젝트와 APPJAM을 처음 경험하는 팀원들만 모여있었기 때문에 역할분담, 일정관리, 이슈관리 등 모든 게 서툴렀었습니다😂 그래서 저는 iOS파트가 역할분담, 일정관리 등을 어떤 식으로 하는지 물어보고 다른 팀 안드로이드 리더 분들께도 조언을 구하며 저희 프로젝트에 적용해나갔습니다.(다시 한번 카드나 iOS 팀원 분들과 도움주신 파트 리더분들 진심으로 고맙습니다🙇‍♀️) 특히 노션을 이용한 타임라인으로 일정을 관리하는 방법이 매우 유용하다고 생각했고, 캘린더와 타임라인에 데드라인과 작업 일정을 기록하며 작업해나갔습니다. 팀원들도 제가 회의를 진행하는 동안 회의록을 작성해주거나 일정을 업데이트 해주는 등 다방면에서 많은 도움을 주었습니다. 덕분에 전체적인 일정관리나 이슈해결 등 프로젝트 전체를 이끌어 가는 일에 집중할 수 있었습니다.
